...
本書は本リポジトリの canonical design document であり、上位基準は存在しない。本文の `Baseline` への参照 (Baseline §N 等) は、かつて制定された上位文書 `docs/spool-design-baseline-v2.md` を指す歴史的参照である。当該文書は現在の public repository には含まれず、矛盾が生じる場合は本書を正本とする。
