...
Status: 実装計画 (2026-09-09)。上位基準は `docs/spool-design-baseline-v2.md` (Baseline)、次いで `DESIGN-v2.md`。(注: Baseline 文書は現在の repository には含まれない。DESIGN-v2.md が現行 canonical 設計)。本書は M2 の範囲抽出と実装順の決定のみを行い、DESIGN が明示していない機能・保証・正本を追加しない。
